...
var CoffeeReviewApp = angular.module('CoffeeReviewApp', ['ngRoute']);

CoffeeReviewApp.config(['$routeProvider',
...
CoffeeReviewApp.controller('CoffeeListCtrl', function($scope, $rootScope) {
$scope.coffees =
       [
       {'id': 1,
       ...
       'reviewer': 'Eileen Dover'
        ...
        $scope.coffeeId='id';
        $scope.onReview = function(coffeeId) {
                for(var i = 0; i < $scope.coffees.length; i++){
                        if($scope.coffees[i].id == coffeeId){
                                $rootScope.currentCoffee = $scope.coffees[i];
                        }
                }
        };
});
